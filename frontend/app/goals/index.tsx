...
export default function GoalsScreen() {
  ...
  const [modalMode, setModalMode] = useState<'create' | 'edit' | 'funds' | null>(null);
  ...
  const handleFunds = async () => {
    ...
    setLoading(true); setErrorMsg('');
    try {
      await goalService.addFunds(editingGoal.id, parseFloat(fundsAmount));
      setModalMode(null); loadData();
    } catch { setErrorMsg('Error al agregar fondos'); }
    finally { setLoading(false); }
  };
  ...
  const handleDelete = (g: SavingsGoal) => {
    Alert.alert('Eliminar', `¿Eliminar "${g.name}"?`, [
      ...
      { text: 'Eliminar', style: 'destructive', onPress: async () => { try { await goalService.delete(g.id); loadData(); } catch {} } },
    ]);
  };
        ...
        {goals.map((g) => (
          <View key={g.id} className="bg-denim rounded-2xl p-4 mb-3">
            <View className="flex-row justify-between items-center mb-3">
              <Text className="text-bone font-semibold text-lg">{g.name}</Text>
            ...
            <View className="flex-row mt-3 pt-3 border-t border-steel/20 gap-2">
              ...
              <TouchableOpacity onPress={() => handleDelete(g)} className="bg-red-500/20 px-3 py-1 rounded-lg"><Text className="text-red-400 text-xs">Eliminar</Text></TouchableOpacity>
            </View>
          </View>
        ))}
      </ScrollView>
