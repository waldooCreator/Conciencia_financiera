...
export default function DashboardScreen() {
  ...
  const handleDelete = async () => {
    ...
    try { await transactionService.delete(deleteTarget.id); setDeleteTarget(null); loadData(); }
      ...
      <ScrollView className="flex-1 p-6" refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor="#20394a" />}>
        ...
        {summary && (
          <View className="flex-row justify-between mb-4">
            <View className="bg-denim rounded-2xl p-4 flex-1 mr-2">
              ...
              <Text className="text-green-400 text-xl font-bold mt-1">${summary.total_income.toLocaleString()}</Text>
            </View>
            <View className="bg-denim rounded-2xl p-4 flex-1 ml-2">
              <Text className="text-concrete text-sm">Gastos</Text>
              <Text className="text-bone text-xl font-bold mt-1">${summary.total_expenses.toLocaleString()}</Text>
            </View>
          </View>
        )}
        ...
        {totalDebt > 0 && (
          <View className="bg-denim rounded-2xl p-4 mb-4">
            <Text className="text-bone font-semibold mb-2">Proyección de Deuda</Text>
            ...
            {goals.map((g) => (
              <View key={g.id} className="bg-denim rounded-2xl p-4 mb-2">
                ...
                <View className="bg-steel/20 rounded-full h-2">
                  <View className="bg-steel rounded-full h-2" style={{ width: Math.min(100, g.progress_percentage) + '%' }} />
                </View>
              </View>
            ))}
          </View>
        ...
        <Text className="text-xl font-bold text-noir mb-3">Transacciones Recientes</Text>
        ...
        {transactions.length > 0 ? (
          transactions.slice(0, 10).map((tx) => (
            <View key={tx.id}>
              ...
              />
              <View className="flex-row justify-end mb-3 -mt-2 px-4">
